...
QT_QPA_PLATFORM=offscreen QT_QPA_PLATFORMTHEME=generic QT_QUICK_CONTROLS_STYLE=Basic \
  XDG_DATA_HOME="$test_dir/data" timeout 20s quickshell -p "$test_dir/shell.qml" 2>&1 | tee "$test_dir/test.log"
rg -q SOLAR_FORGE_TESTS_PASSED "$test_dir/test.log"
if rg -q 'SOLAR_FORGE_TESTS_FAILED|TypeError|ReferenceError|ERROR' "$test_dir/test.log"; then exit 1; fi
